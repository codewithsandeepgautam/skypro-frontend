...
const BlogsDetails = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await GetRequest(`${process.env.REACT_APP_API_URL}/blogs/getblogs`);
        ...
        if (!foundBlog) {
          ...
          const selectedBlog = blogsData.find(
            (blog) => blog.handle === foundBlog.handle
          );
      ...
      <SEO title={blogDetails?.metaTitle} description={blogDetails?.metaDescription} keywords={blogDetails?.keyword}
        schema={
          `{
                        ...
                        "mainEntityOfPage": {
                          "@type": "WebPage",
                          "@id": "https://www.skypro.co.in/blogs/${blogDetails?.handle}"
                        },
                        "headline": "${blogDetails?.title}",
                        "image": [
                          "${blogDetails?.image}"
                        ],
                        "datePublished": "${blogDetails?.publishedAt}",
                        "author": {
                          "@type": "Person",
                        ...
                        "publisher": {
                          ...
                          "logo": {
                            "@type": "ImageObject",
                            "url": "https://res.cloudinary.com/devh5cxfo/image/upload/v1717410068/stfp14svvgen5vwyvg2u.webp"
                          }
                        },
            ...
            <div className="w-[32%] max-lg:w-full">
              ...
              <div className="py-[40px] text-white px-[30px] bg-[linear-gradient(157deg,#061e43_0%,#3ad8ff_100%)] rounded-[22px]">
                ...
                <Link
                  ...
                  target="_blank"
                  className="flex mt-6 w-48 h-14 bg-white text-black rounded-lg items-center justify-center"
                >
                  <div className="mr-3">
