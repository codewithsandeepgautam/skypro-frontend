...
const Package = () => {
  ...
  const handlePackageData = useCallback(
    ...
    [navigate, setPackageData]
  );
  return (
    <>
      <SEO
        title="Affordable IPTV Subscription Packages for Indian Channels"
        description="Get IPTV subscription plans with HD channels. Find Indian IPTV packages and subscription options featuring a wide range of channels at affordable pricing."
        keywords="IPTV Indian Package, iptv package, iptv subscription plans, iptv pro subscription, IPTV Service Subscription, hd iptv packages, iptv subscription packages, iptv packages, indian iptv subscription, iptv india subscription, IPTV paid subscription, subscription IPTV, IPTV subscription channel list"
      />
      <InnerPageBanner title='Packages' innerPageBanner={ImageIcons.iptvAppBg2} />
      <section>
                  ...
                  <div key={index} className='shadow-[0px_2px_20px_0px_rgba(0,0,0,0.15)] rounded-[40px] group '>
                    <div className="rounded-lg text-center text-black">
                      ...
                      <div className='py-[40px]'>
                        ...
                        <div className='border border-[#fd5901] text-[#fd5901] cursor-pointer hover:text-[#fff] hover:bg-[#fd5901] inline-flex py-[9px] px-[20px] rounded-[10px] text-[17px]' onClick={() => { handlePackageData(item?.name) }}>Add Pack</div>
                ...
                {packages?.isaddon.map((item, index) => (
                  <div key={index} className='shadow-[0px_2px_20px_0px_rgba(0,0,0,0.15)] rounded-[40px] group '>
                    <div className="rounded-lg text-center text-black">
                      ...
                      <div className='py-[40px]'>
                        <h2 className="mb-[15px]">₹{item?.price} +GST</h2>
                        <div className='border border-[#fd5901] cursor-pointer text-[#fd5901] hover:text-[#fff] hover:bg-[#fd5901] inline-flex py-[9px] px-[20px] rounded-[10px] text-[17px]' onClick={() => { handlePackageData(item?.name) }}>Add Pack</div>
                        <Link to={item?.pdfFile} target="_blank" className='border border-[#071e43] text-[#071e43] hover:text-[#fff] hover:bg-[#071e43] inline-flex py-[9px] px-[20px] rounded-[10px] text-[17px] ml-[10px] flex justify-center gap-[6px] items-center' onClick={windowScroll}>View <ImFilePdf /></Link>
                      </div>
                    </div>
                  </div>
      ...
      {/* <section className='bg-[#f3f5f7]'>
        <div className='container'>
          <div className='group bg-[#fff] p-[25px] mb-[20px]'>
            <h2 className='group-hover:text-[#fd5901] duration-500 max-sm:text-[28px]'>Skypro IPTV Subscription Packages</h2>
