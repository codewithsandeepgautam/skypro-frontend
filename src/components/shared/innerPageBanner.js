...
const InnerPageBanner = ({ title = "", heading = "", innerPageBanner = "", path = "", handle = "" }) => {
  return (
    <section
    ...
    >
      <div className="heading_tittle container relative z-10">
        ...
        {heading && (
          <h2 className="capitalize text-white text-center text-[45px] tracking-[2px] drop-shadow-[0_0_5px_#00000052] mb-0">
            {heading}
          </h2>
        )}
          ...
          <ul>
            {handle ? (
              <li className="inline-block">
                <Link className="text-white font-medium hover:text-[#]" to="/home">
                  {handle}
                </Link>
              </li>
            ) :
